...
uri_europe = URI.parse('https://data.opendatasoft.com/api/records/1.0/search/?dataset=world-heritage-list%40public-us&rows=30&facet=category&facet=region&facet=states&refine.region=Europe+and+North+America')
json_europe = Net::HTTP.get(uri_europe) #NET::HTTPを利用してAPIを呼ぶ
result_europe = JSON.parse(json_europe) #返ってきたjsonデータをrubyの配列に変換するためのライン
sites_europe = result_europe['records']
